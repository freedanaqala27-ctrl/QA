...
def render_welcome() -> None:
    ...
    if st.button("开始填写", width="stretch"):
...
def render_consent(sequence: list[str]) -> None:
    ...
    back = col1.button("返回", width="stretch")
    next_step = col2.button("继续", width="stretch")
    if back:
...
def render_background(sequence: list[str]) -> None:
    ...
    with st.form("background_form"):
        ...
        col1, col2 = st.columns(2)
        back = col1.form_submit_button("返回", width="stretch")
        next_step = col2.form_submit_button("保存并开始答题", width="stretch")
    if back:
        move_page(-1, sequence)
        st.rerun()
    if next_step:
...
def render_item(sequence: list[str], package_df: pd.DataFrame, blind_exercise_id: str) -> None:
    ...
    st.title(f"第 {display_order} / {total_items} 题")
    render_exercise(row)
    ...
    with st.form(f"item_form_{blind_exercise_id}"):
        ...
        st.text_area(
            "如果只能修改一个地方，你最希望改哪里？（可选）",
            key=f"{blind_exercise_id}_open_comment",
        )
        col1, col2 = st.columns(2)
        back = col1.form_submit_button("上一题", width="stretch")
        next_step = col2.form_submit_button("保存并继续", width="stretch")
...
def render_attention(sequence: list[str]) -> None:
    ...
    with st.form("attention_form"):
        ...
        col1, col2 = st.columns(2)
        back = col1.form_submit_button("上一页", width="stretch")
        next_step = col2.form_submit_button("保存并继续", width="stretch")
    if back or next_step:
        save_attention()
        move_page(-1 if back else 1, sequence)
...
def render_batch(sequence: list[str]) -> None:
    st.title("整体评价")
    st.write("以下题目请基于你刚刚完成的这一批练习题整体体验作答。")
    st.caption(LIKERT_CAPTION)
    with st.form("batch_form"):
        ...
        col1, col2 = st.columns(2)
        back = col1.form_submit_button("上一页", width="stretch")
        submit = col2.form_submit_button("提交问卷", width="stretch")
    if back:
        move_page(-1, sequence)
        st.rerun()
    if submit:
